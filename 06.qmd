...
admissions |>
  ...
  summarize(total_apps = sum(APPLCN, na.rm = TRUE)) |>   # compute the height ourselves
  ggplot(aes(x = adm_tscores, y = total_apps)) +
  geom_col() +                                           # bars use the summed value as height
  coord_flip()                                           # flip sideways so labels fit
```
