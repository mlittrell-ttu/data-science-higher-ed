...
: First, the dataset is loaded into a tibble called *enrollment*. Note that my dataset is stored in a folder called *data*, so my path starts with `data/`. If your data are in the same folder as your script, just use the filename `("7160_module1_student_enrollment.csv")`.
```{r}
...
: head() shows the first six rows of the data giving an at-a-glance picture of what is there. Also running **view()** will show you your data in RStudio via a tabular browser.
```{r}
head(enrollment)      
```
...
: Here we confirm the change with glimpse() again.
```{r}
glimpse(enrollment)
```
